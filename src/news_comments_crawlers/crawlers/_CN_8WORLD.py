...
css_paths = {
    ...
    'category': 'ul > li[class="topic"]',
    'date': 'div[class*="field--type-datetime"] > time',
    'article':  'div[class="text-long"]'
    }

def extract_datetime_8w(raw_date_string, crawl):
    
    raw_date_string = re.sub('\s+',' ',raw_date_string)
    
    try:
  
        date_time = re.findall(r'\d+\W\d+\W\d+\s\d+\W\d+', raw_date_string)[0]
