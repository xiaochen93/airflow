...
DEFAULT_WEBSITE = 'https://stomp.straitstimes.com/'

DEFAULT_DOMAIN = 'https://stomp.straitstimes.com/'

SITEMAP = 'https://stomp.straitstimes.com/sitemap.xml'
...
def gather_urls(save_to_cache = False):
    ...
    for sitemap_page in range(SITEMAP_NUM_PAGES):
...
def _getNewsByCSS(article_url,css_paths,allow_redirects=True):
    ...
    soup = BeautifulSoup(page_content.text, 'html')
    ...
    try:
        category = ' '.join([each.get_text() for each in soup.select(css_paths['category'])])
    except Exception as e:
        category = ''
...
def main():
    ...
    articles_df = Preprocessor({'data':articles,'lang':LANG,'org':1, 'source': 16}).preprocess(func=_extract_datetime)
